...
(() => {
  // No tocar ========================================================
  const SW_API = 'https://swapi.py4e.com/api';
  const getRequest = (url: string) => ajax.getJSON<any>(url);
  // ==================================================================

  const character$ = getRequest(`${SW_API}/people/1/`);
  const species$ = character$.pipe(pluck('species'), concatMap(getRequest));

  zip([character$, species$])
    .pipe(map(([specie, personaje]) => ({ specie, personaje })))
